...
class QueueElement {
  ...
  QueueElement({required this.cost, required this.id});
}

class PriorityQueue {
  SplayTreeSet<QueueElement> queue = SplayTreeSet<QueueElement>((el1, el2) {
    final costDiff = el1.cost.compareTo(el2.cost);
    return costDiff == 0 ? el1.id.compareTo(el2.id) : costDiff;
  });
  PriorityQueue();
  ...
  add(value, cost) {
    var item = QueueElement(id: value, cost: cost);
    queue.add(item);
  }
